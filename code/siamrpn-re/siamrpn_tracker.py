...
class SiamRPN(nn.Module):
    def __init__(self, anchor_num=5):
        ...
        self.conv_reg_temp = nn.Conv2d(512, 512 * 4 * anchor_num, 3, 1)
        self.conv_reg_det = nn.Conv2d(512, 512, 3)
        self.conv_cls_temp = nn.Conv2d(512, 512 * 2 * anchor_num, 3, 1)
        self.conv_cls_det = nn.Conv2d(512, 512, 3)
        self.adjust_reg = nn.Conv2d(4 * anchor_num, 4 * anchor_num, 1)

    def forward(self, temp, det):
        z = self.feature(temp)
        x = self.feature(det)
